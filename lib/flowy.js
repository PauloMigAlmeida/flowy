export default function flowy(canvas_div, grab, release, snapping, rearrange, spacing_x, spacing_y) {
    ...
    flowy.load = function () {
        ...
        flowy.beginDrag = function (event) {
            ...
            const canvas_div_css_position = window.getComputedStyle(canvas_div).position;
            if (canvas_div_css_position === "absolute" || canvas_div_css_position === "fixed") {
                absx = canvas_div.getBoundingClientRect().left;
                absy = canvas_div.getBoundingClientRect().top;
            }
            ...
            if (event.which != 3 && event.target.closest(".create-flowy")) {
                ...
                const next_block_id = blocks.map(a => a.id).reduce((a, b) => Math.max(a, b), -1) + 1;
                newNode.innerHTML += "<input type='hidden' name='blockid' class='blockid' value='" + next_block_id + "'>";
                document.body.appendChild(newNode);
                drag = document.querySelector(".blockid[value='" + next_block_id + "']").parentNode;
                ...
                if (parseInt(drag.querySelector(".blockid").value) === 0 && rearrange) {
                    ...
                    for (var i = 0; i < blocks.length; i++) {
                        if (checkAttach(blocko[i])) {
                            ...
                            if (beforeDelete(drag, blocks.filter(a => a.id === blocko[i])[0])) {
                                active = false;
                                drag.classList.remove("dragging");
        ...
        function checkAttach(id) {
            ...
            const blockOfInterest = blocks.filter(a => a.id == id)[0];
            return xpos >= blockOfInterest.x - (blockOfInterest.width / 2) - paddingx
                && xpos <= blockOfInterest.x + (blockOfInterest.width / 2) + paddingx
                && ypos >= blockOfInterest.y - (blockOfInterest.height / 2)
                && ypos <= blockOfInterest.y + blockOfInterest.height;
        }
        ...
        function drawArrow(arrow, x, y, id) {
            let filterElement = blocks.filter(a => a.id === id)[0];
            if (x < 0) {
                canvas_div.innerHTML += '' +
                    '<div class="arrowblock">' +
                    '   <input type="hidden" class="arrowid" value="' + drag.querySelector(".blockid").value + '">' +
                    '   <svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                    '       <path d="M' + (filterElement.x - arrow.x + 5) + ' 0L' + (filterElement.x - arrow.x + 5) + ' ' + (paddingy / 2) + 'L5 ' + (paddingy / 2) + 'L5 ' + y + '" stroke="#C5CCD0" stroke-width="2px"/>' +
                ...
                canvas_div.innerHTML += '' +
                    ...
                    '   <input type="hidden" class="arrowid" value="' + drag.querySelector(".blockid").value + '">' +
                    '   <svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                    '       <path d="M20 0L20 ' + (paddingy / 2) + 'L' + (x) + ' ' + (paddingy / 2) + 'L' + x + ' ' + y + '" stroke="#C5CCD0" stroke-width="2px"/>' +
                    '       <path d="M' + (x - 5) + ' ' + (y - 5) + 'H' + (x + 5) + 'L' + x + ' ' + y + 'L' + (x - 5) + ' ' + (y - 5) + 'Z" fill="#C5CCD0"/>' +
                    '   </svg>' +
                    '</div>';
                document.querySelector('.arrowid[value="' + parseInt(drag.querySelector(".blockid").value) + '"]').parentNode.style.left = filterElement.x - 20 - (absx + window.scrollX) + canvas_div.scrollLeft + canvas_div.getBoundingClientRect().left + "px";
            }
            document.querySelector('.arrowid[value="' + parseInt(drag.querySelector(".blockid").value) + '"]').parentNode.style.top = filterElement.y + (filterElement.height / 2) + canvas_div.getBoundingClientRect().top - absy + "px";
        }

        function updateArrow(arrow, x, y, children) {
            let filterElement = blocks.filter(id => id.id == children.parent)[0];
            if (x < 0) {
                ...
                document.querySelector('.arrowid[value="' + children.id + '"]').parentNode.style.left = filterElement.x - 20 - (absx + window.scrollX) + canvas_div.getBoundingClientRect().left + "px";
                document.querySelector('.arrowid[value="' + children.id + '"]').parentNode.innerHTML = '' +
                    '<input type="hidden" class="arrowid" value="' + children.id + '">' +
                    '<svg preserveaspectratio="none" fill="none" xmlns="http://www.w3.org/2000/svg">' +
                    '   <path d="M20 0L20 ' + (paddingy / 2) + 'L' + (x) + ' ' + (paddingy / 2) + 'L' + x + ' ' + y + '" stroke="#C5CCD0" stroke-width="2px"/>' +
        ...
        flowy.moveBlock = function (event) {
            ...
            if (active || rearrange) {
                ...
                // I can possible get rid of this blocko shit
                var blocko = blocks.map(a => a.id);
                for (var i = 0; i < blocks.length; i++) {
                    if (checkAttach(blocko[i])) {
    ...
    function blockGrabbed(block) {
        console.log('blockGrabbed');
        grab(block);
    }

    function blockReleased() {
        console.log('blockReleased');
        release();
    }
